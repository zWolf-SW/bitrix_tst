...
/** @var \Bitrix\MessageService\Sender\Sms\MfmsRu $sender */
$sender = $arResult['sender'];

?>
<div class="sms-settings">
	<h2 class="sms-settings-title">MFMS.ru</h2>
		...
		<?php if (!$sender->isRegistered()):?>
			<div class="sms-settings-step-section">
				...
				<form action="" method="post" class="sms-settings-step-form" name="form_sms_registration">
					...
					<div class="sms-settings-input-container">
						<label for="" class="sms-settings-input-label">Password*</label>
						<input type="text" name="password" class="sms-settings-input">
					</div>
					<div class="sms-settings-input-container">
						<label for="" class="sms-settings-input-label">HPG Send Url*</label>
						<input type="text" name="hpg_send_url" class="sms-settings-input">
					</div>
					<div class="sms-settings-input-container">
						<label for="" class="sms-settings-input-label">HPG Status Url**</label>
						<input type="text" name="hpg_status_url" class="sms-settings-input">
					</div>
					<div class="sms-settings-input-container">
						<label for="" class="sms-settings-input-label">Allowed From (<b>;</b>-separated)*</label>
						<input type="text" name="from_list" class="sms-settings-input">
					</div>
					<div class="sms-settings-input-container">
		...
		<?else:
			...
			<div class="sms-settings-step-section sms-settings-step-section-active">
				...
				<div class="sms-settings-step-contact-info">
					...
					<div class="sms-settings-step-contact-info-block">
						<div class="sms-settings-step-contact-info-name">From list:</div>
						<div class="sms-settings-step-contact-info-value"><?=htmlspecialcharsbx(implode('; ', array_column($ownerInfo['fromList'],'name')))?></div>
					</div>
				</div>
			</div>
			<!---->
	...
	BX.ready(function()
	{
		...
		if (registrationForm)
		{
			...
			BX.bind(registrationForm, 'submit', function(e)
			{
				...
				var pass = registrationForm.elements['password'].value;
				var fromList = registrationForm.elements['from_list'].value;
				var sendUrl = registrationForm.elements['hpg_send_url'].value;
				var statusUrl = registrationForm.elements['hpg_status_url'].value;

				if (!login.length || !pass.length || !fromList.length || !sendUrl.length || !statusUrl.length)
				{
