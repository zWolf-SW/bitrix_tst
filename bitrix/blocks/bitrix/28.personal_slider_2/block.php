<section class="landing-block js-animation fadeIn g-pt-20 g-pb-20">
	<div class="landing-block-slider container" data-slider-dots="0" data-slider-arrows="7">
		<div class="js-carousel"
			 data-infinite="true"
			 data-fade="true"
			 data-lazy-load="ondemand"
			 data-arrows-classes="u-arrow-v1 g-absolute-centered--y g-width-45 g-height-45 g-font-size-30 g-transition-0_2 g-transition--ease-in g-color-gray-light-v1 g-color-primary-dark-v2--hover"
			 data-arrow-left-classes="fa fa-angle-left g-left-0 g-left-40--lg"
			 data-pagi-classes="u-carousel-indicators-v1 g-absolute-centered--x g-bottom-0"
			 data-arrow-right-classes="fa fa-angle-right g-right-0 g-right-40--lg">
			<div class="landing-block-card-person js-slide align-self-center g-pt-30 g-pb-30 g-pl-40 g-pr-40 g-pr-10--md g-pl-10--lg">
				<div class="landing-block-container row justify-content-center align-items-center no-gutters">
					<div class="col-sm-6 col-lg-4">
						<img class="landing-block-node-person-photo img-fluid g-pb-10 g-pb-0--md"
							 src="https://cdn.bitrix24.site/bitrix/images/landing/business/400x450/img1.jpg" alt="">
					</div>
					<div class="col-sm-6 col-lg-4">
						<div class="g-px-0 g-px-30--md g-px-50--lg">
							<h4 class="landing-block-node-person-name mb-1 g-color">David Case</h4>
							<div class="landing-block-node-person-post g-color">
								<span class="d-block mb-4">Web Developer</span>
							</div>
							<div class="landing-block-node-person-text mb-4 g-color">
								<p>I am a 32-year old man from Canada and I am passionate about everything related to web development. I strive to figure out the right solutions for your look to stand out amongst the rest.</p>
							</div>
							<a class="landing-block-node-person-link u-link-v5 g-color-text g-font-weight-700 g-color" href="#" style="--color: var(--primary);">See Projects</a>
						</div>
					</div>
				</div>
			</div>

			<div class="landing-block-card-person js-slide align-self-center g-pt-30 g-pb-30 g-pl-40 g-pr-40 g-pr-10--md g-pl-10--lg">
				<div class="landing-block-container row justify-content-center align-items-center no-gutters">
					<div class="col-sm-6 col-lg-4">
						<img class="landing-block-node-person-photo img-fluid g-pb-10 g-pb-0--md"
							 src="https://cdn.bitrix24.site/bitrix/images/landing/business/400x450/img2.jpg" alt="">
					</div>
					<div class="col-sm-6 col-lg-4">
						<div class="g-px-0 g-px-30--md g-px-50--lg">
							<h4 class="landing-block-node-person-name mb-1 g-color">Jessica Lisbon</h4>
							<div class="landing-block-node-person-post g-color">
								<span class="d-block mb-4">Support Manager</span>
							</div>
							<div class="landing-block-node-person-text mb-4 g-color">
								<p>I've got your front end and back end needs covered. I'm always looking for strategies to make your brand's needs fit with today's development languages.</p>
							</div>
							<a class="landing-block-node-person-link u-link-v5 g-color-text g-font-weight-700 g-color" href="#" style="--color: var(--primary);">See Projects</a>
						</div>
					</div>
				</div>
			</div>

			<div class="landing-block-card-person js-slide align-self-center g-pt-30 g-pb-30 g-pl-40 g-pr-40 g-pr-10--md g-pl-10--lg">
				<div class="landing-block-container row justify-content-center align-items-center no-gutters">
					<div class="col-sm-6 col-lg-4">
						<img class="landing-block-node-person-photo img-fluid g-pb-10 g-pb-0--md"
							 src="https://cdn.bitrix24.site/bitrix/images/landing/business/400x450/img3.jpg" alt="">
					</div>
					<div class="col-sm-6 col-lg-4">
						<div class="g-px-0 g-px-30--md g-px-50--lg">
							<h4 class="landing-block-node-person-name mb-1 g-color">Monica Gaudy</h4>
							<div class="landing-block-node-person-post g-color">
								<span class="d-block mb-4">Sales Manager</span>
							</div>
							<div class="landing-block-node-person-text mb-4 g-color">
								<p>I am Monica and I aim high at being focused on building relationships with our clients and community.</p>
							</div>
							<a class="landing-block-node-person-link u-link-v5 g-color-text g-font-weight-700 g-color" href="#" style="--color: var(--primary);">See Projects</a>
						</div>
					</div>
				</div>
			</div>

			<div class="landing-block-card-person js-slide align-self-center g-pt-30 g-pb-30 g-pl-40 g-pr-40 g-pr-10--md g-pl-10--lg">
				<div class="landing-block-container row justify-content-center align-items-center no-gutters">
					<div class="col-sm-6 col-lg-4">
						<img class="landing-block-node-person-photo img-fluid g-pb-10 g-pb-0--md"
							 src="https://cdn.bitrix24.site/bitrix/images/landing/business/400x450/img4.jpg" alt="">
					</div>
					<div class="col-sm-6 col-lg-4">
						<div class="g-px-0 g-px-30--md g-px-50--lg">
							<h4 class="landing-block-node-person-name mb-1 g-color">Tina Krueger</h4>
							<div class="landing-block-node-person-post g-color">
								<span class="d-block mb-4">Lead Designer</span>
							</div>
							<div class="landing-block-node-person-text mb-4 g-color">
								<p>I'm Tina Krueger, an excitable lead designer. I live with an intense passion for web development.</p>
							</div>
							<a class="landing-block-node-person-link u-link-v5 g-color-text g-font-weight-700 g-color" href="#" style="--color: var(--primary);">See Projects</a>
						</div>
					</div>
				</div>
			</div>

			<div class="landing-block-card-person js-slide align-self-center g-pt-30 g-pb-30 g-pl-40 g-pr-40 g-pr-10--md g-pl-10--lg">
				<div class="landing-block-container row justify-content-center align-items-center no-gutters">
					<div class="col-sm-6 col-lg-4">
						<img class="landing-block-node-person-photo img-fluid g-pb-10 g-pb-0--md"
							 src="https://cdn.bitrix24.site/bitrix/images/landing/business/400x450/img5.jpg" alt="">
					</div>
					<div class="col-sm-6 col-lg-4">
						<div class="g-px-0 g-px-30--md g-px-50--lg">
							<h4 class="landing-block-node-person-name mb-1 g-color">John Watson</h4>
							<div class="landing-block-node-person-post g-color">
								<span class="d-block mb-4">Marketing Manager</span>
							</div>
							<div class="landing-block-node-person-text mb-4 g-color">
								<p>Understanding who you are and what you want is my strategy for your brand. I am always figuring out ways to capture your vision, so people can get on board.</p>
							</div>
							<a class="landing-block-node-person-link u-link-v5 g-color-text g-font-weight-700 g-color" href="#" style="--color: var(--primary);">See Projects</a>
						</div>
					</div>
				</div>
			</div>

			<div class="landing-block-card-person js-slide align-self-center g-pt-30 g-pb-30 g-pl-40 g-pr-40 g-pr-10--md g-pl-10--lg">
				<div class="landing-block-container row justify-content-center align-items-center no-gutters">
					<div class="col-sm-6 col-lg-4">
						<img class="landing-block-node-person-photo img-fluid g-pb-10 g-pb-0--md"
							 src="https://cdn.bitrix24.site/bitrix/images/landing/business/400x450/img6.jpg" alt="">
					</div>
					<div class="col-sm-6 col-lg-4">
						<div class="g-px-0 g-px-30--md g-px-50--lg">
							<h4 class="landing-block-node-person-name mb-1 g-color">Maria Olsson</h4>
							<div class="landing-block-node-person-post g-color">
								<span class="d-block mb-4">Technical Director</span>
							</div>
							<div class="landing-block-node-person-text mb-4 g-color">
								<p>I am an ambitious workaholic, but apart from that, pretty simple person. Whether it's branding, print, UI + UX I've got you covered.</p>
							</div>
							<a class="landing-block-node-person-link u-link-v5 g-color-text g-font-weight-700 g-color" href="#" style="--color: var(--primary);">See Projects</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>