...
class CAllSearch extends CDBResult
{
	...
	var $error = false;
	var $arParams = [];
	var $url_add_params = []; //additional url params (OnSearch event)
	var $tf_hwm = 0;
	var $tf_hwm_site_id = '';
	...
	function __construct($strQuery = false, $SITE_ID = false, $MODULE_ID = false, $ITEM_ID = false, $PARAM1 = false, $PARAM2 = false, $aSort = [], $aParamsEx = [], $bTagsCloud = false)
	{
	...
	function CSearch($strQuery = false, $LID = false, $MODULE_ID = false, $ITEM_ID = false, $PARAM1 = false, $PARAM2 = false, $aSort = [], $aParamsEx = [], $bTagsCloud = false)
	{
		if ($strQuery === false)
		{
			return $this;
	...
	function Search($arParams, $aSort = [], $aParamsEx = [], $bTagsCloud = false)
	{
		$DB = CDatabase::GetModuleConnection('search');

		if (!is_array($arParams))
		{
			$arParams = ['QUERY' => $arParams];
		}
		...
		else
		{
			...
			$strQuery = preg_replace_callback('/&#(\\d+);/', [$this, 'chr'], $strQuery);
			$bTagsSearch = false;
		}
		...
		$query = $this->Query->GetQueryString('sct.SEARCHABLE_CONTENT', $strQuery, $bTagsSearch, $aParamsEx['STEMMING'], $this->_opt_ERROR_ON_EMPTY_STEM);
		...
		foreach (GetModuleEvents('search', 'OnSearch', true) as $arEvent)
		{
			...
			if ($bTagsSearch)
			{
				if ($strTags <> '')
				{
					$r = ExecuteModuleEventEx($arEvent, ['tags:' . $strTags]);
				}
			}
			else
			{
				$r = ExecuteModuleEventEx($arEvent, [$strQuery]);
			}
			if ($r <> '')
			{
	...
	function PrepareSearchResult($str)
	{
		...
		$arPos = []; //This will contain positions of the first occurrence
		$arPosW = []; //This is "running" words array
		$arPosP = []; //and their positions
	...
	public static function ReIndexAll($bFull = false, $max_execution_time = 0, $NS = [], $clear_suggest = false)
	{
		global $APPLICATION;
		$DB = CDatabase::GetModuleConnection('search');
		...
		{
			$NS_OLD = $NS;
			$NS = ['CLEAR' => 'N', 'MODULE' => '', 'ID' => '', 'SESS_ID' => md5(uniqid(''))];
			if ($NS_OLD['SITE_ID'] != '')
			{
			...
			if ($NS_OLD['MODULE_ID'] != '')
			{
				$NS['MODULE_ID'] = $NS_OLD['MODULE_ID'];
			}
		}
		$NS['CNT'] = intval($NS['CNT']);
		if (!$bFull && (!isset($NS['SESS_ID']) || mb_strlen($NS['SESS_ID']) != 32))
		{
		...
		if (!isset($NS['CLEAR']) || $NS['CLEAR'] != 'Y')
		{
			if ($bFull)
			{
				...
				CAdminNotify::DeleteByTag('SEARCH_REINDEX');
			}
		...
		if (
			(!isset($NS['MODULE']) || $NS['MODULE'] == '' || $NS['MODULE'] == 'main')
			&& (!isset($NS['MODULE_ID']) || $NS['MODULE_ID'] == '' || $NS['MODULE_ID'] == 'main')
		)
		{
			$arLangDirs = [];
			$arFilter = ['ACTIVE' => 'Y'];
			if (isset($NS['SITE_ID']) && $NS['SITE_ID'] != '')
			{
			...
			foreach ($arLangDirs as $arR)
			{
				...
				//for every folder
				CSearch::RecurseIndex([$site, $path], $max_execution_time, $NS);
				if (
					$max_execution_time > 0
					&& $NS['MODULE'] <> ''
				)
		...
		foreach (GetModuleEvents('search', 'OnReindex', true) as $arEvent)
		{
			if (
				isset($NS['MODULE_ID'])
				&& $NS['MODULE_ID'] != ''
				&& $NS['MODULE_ID'] != $arEvent['TO_MODULE_ID']
			)
			{
				continue;
			}
			if (
				...
				&& $NS['MODULE'] <> ''
				&& $NS['MODULE'] != 'main'
				&& $NS['MODULE'] != $arEvent['TO_MODULE_ID']
			)
			{
				continue;
			}
			...
			$r = &$oCallBack;
			$arResult = ExecuteModuleEventEx($arEvent, [$NS, $r, 'Index']);
	...
	public static function ReindexModule($MODULE_ID, $bFull = false)
	{
		...
		foreach (GetModuleEvents('search', 'OnReindex', true) as $arEvent)
		{
			if ($arEvent['TO_MODULE_ID'] != $MODULE_ID)
			{
				continue;
			}
			...
			{
				return ['MODULE' => $arEvent['TO_MODULE_ID'], 'CNT' => $oCallBack->CNT, 'ID' => $arResult, 'CLEAR' => $NS['CLEAR'], 'SESS_ID' => $NS['SESS_ID']];
			}
		}
	...
	public static function Index($MODULE_ID, $ITEM_ID, $arFields, $bOverWrite = false, $SEARCH_SESS_ID = '')
	{
		...
		{
			$arEventResult = ExecuteModuleEventEx($arEvent, [$arFields]);
		...
		if (array_key_exists('SITE_ID', $arFields))
		{
			...
			{
				$arFields['SITE_ID'] = [$arFields['SITE_ID'] => ''];
			}
			else
			{
				$bNotAssoc = true;
		...
		if ($arResult = $r->Fetch())
		{
			...
			if ($bTitle && $bBody && $arFields['BODY'] == '' && $arFields['TITLE'] == '')
			{
				...
				{
					ExecuteModuleEventEx($arEvent, ['SEARCH_CONTENT_ID = ' . $ID]);
				}
			...
			if (count($arGroups) > 0)
			{
				CSearch::SetContentItemGroups($ID, $arGroups);
			}
			...
			// Avoid double update of related tables
			unset($arFields['PARAMS']);
			unset($arFields['PERMISSIONS']);
			unset($arFields['SITE_ID']);

			// This fields are needed for full text update
	...
	public static function ReindexFile($path, $SEARCH_SESS_ID = '')
	{
		...
		{
			if ($arrFile = ExecuteModuleEventEx($arEvent, [$file_abs_path, $SEARCH_SESS_ID]))
			{
				break;
			}
		}
		...
		$title = mb_convert_encoding($title, 'UTF-8', 'UTF-8'); // Remove invalid utf
		if ($title == '')
		{
			return 0;
		}
		...
		$filesrc = mb_convert_encoding($filesrc, 'UTF-8', 'UTF-8'); // Remove invalid utf
		$arGroups = CSearch::GetGroupCached();
		$arGPerm = [];
		foreach ($arGroups as $group_id)
		{
			$p = $APPLICATION->GetFileAccessPermission([$file_site, $file_rel_path], [$group_id]);
			if ($p >= 'R')
			{
				$arGPerm[] = $group_id;
				if ($group_id == 2)
	...
	public static function RecurseIndex($path = [], $max_execution_time = 0, &$NS)
	{
		if (!is_array($path))
		{
			return 0;
		...
		foreach ($d->GetChildren() as $dir_entry)
		{
			...
			if ($dir_entry->IsDirectory())
			{
				...
				)
				{
					...
					{
						if (CSearch::RecurseIndex([$site, $path_file], $max_execution_time, $NS) === false)
						{
							return false;
						}
					}
			...
			else
			{
				...
				elseif (!isset($NS['MODULE']) || $NS['MODULE'] == '')
				{
					$ID = CSearch::ReindexFile([$site, $path_file], $NS['SESS_ID'] ?? '');
					if (intval($ID) > 0)
					{
	...
	public static function __PrepareFilter($arFilter, &$bIncSites, $strSearchContentAlias = 'sc.')
	{
		...
		foreach ($arFilter as $field => $val)
		{
			...
			default:
				...
				{
					$sql = ExecuteModuleEventEx($arEvent, [$strSearchContentAlias, $field, $val]);
					if ($sql <> '')
					{
						$arSql[] = '(' . $sql . ')';
						break;
		...
		$obQueryWhere->SetFields([
			...
			'URL' => [
				...
				'MULTIPLE' => 'N',
				'FIELD_TYPE' => 'callback',
				'CALLBACK' => [$obWhereHelp, '_CallbackURL'],
				'JOIN' => true,
			],
			'PARAMS' => [
				'TABLE_ALIAS' => $strSearchContentAlias,
				...
				'MULTIPLE' => 'N',
				'FIELD_TYPE' => 'callback',
				'CALLBACK' => [$obWhereHelp, '_CallbackPARAMS'],
				'JOIN' => false,
			],
		]);
	...
	function __PrepareSort($aSort = [], $strSearchContentAlias = 'sc.', $bTagsCloud = false)
	{
		...
		else
		{
			...
			if (count($arOrder) == 0)
			{
				...
				$arOrder[] = $helper->quote('RANK') . ' DESC';
	...
	public static function Add($arFields)
	{
		...
		$tableFields = [];
		foreach ($DB->GetTableFields('b_search_content') as $field)
		{
			$tableFields[$field['NAME']] = $field['TYPE'];
		}

		foreach ($arFields as $fieldName => $_)
		{
			$search = str_ends_with($fieldName, '~') ? mb_substr($fieldName, 0, -1) : $fieldName;
			if (!isset($tableFields[$search]))
			{
				unset($arFields[$fieldName]);
			}
		}

		if (isset($arFields['DATE_CHANGE']))
		{
			$arFields['DATE_CHANGE'] = new \Bitrix\Main\Type\DateTime($arFields['DATE_CHANGE']);
		}
		...
		if (isset($arFields['DATE_FROM']))
		{
			...
			{
				$arFields['DATE_FROM'] = new \Bitrix\Main\Type\DateTime($arFields['DATE_FROM']);
			}
		}

		if (isset($arFields['DATE_TO']))
		{
			if ($arFields['DATE_TO'])
			{
				$arFields['DATE_TO'] = new \Bitrix\Main\Type\DateTime($arFields['DATE_TO']);
			}
		}
		...
		$ar = $DB->Query('SELECT ID FROM b_search_content WHERE MODULE_ID=\'' . $DB->ForSql($arFields['MODULE_ID']) . '\' AND ITEM_ID=\'' . $DB->ForSql($arFields['ITEM_ID']) . '\'')->Fetch();

		return $ar ? $ar['ID'] : false;
	}

	public static function OnChangeFilePermissions($path, $permission = [], $old_permission = [], $arGroups = false)
	{
		...
		if (file_exists($DOC_ROOT . $path))
		{
			...
			if (is_dir($DOC_ROOT . $path))
			{
				...
				while (false !== ($file = @readdir($handle)))
				{
					...
					{
						CSearch::OnChangeFilePermissions([$site, $full_file], [], [], $arGroups);
					}
				}
			}
			else
			{
				...
				if ($ar = $rs->Fetch())
				{
					...
					foreach ($arGroups as $group_id)
					{
						$p = $APPLICATION->GetFileAccessPermission([$site, $path], [$group_id]);
						if ($p >= 'R')
						{
					...
					CSearch::SetContentItemGroups($ar['ID'], $arNewGroups);
				}
			}
		}
	}
	...
	public static function DeleteOld($SESS_ID, $MODULE_ID = '', $SITE_ID = '')
	{
		...
		while ($ar = $rs->Fetch())
		{
			...
			{
				ExecuteModuleEventEx($arEvent, ['SEARCH_CONTENT_ID = ' . $ar['ID']]);
			}
	...
	public static function DeleteIndex($MODULE_ID, $ITEM_ID = false, $PARAM1 = false, $PARAM2 = false, $SITE_ID = false)
	{
		...
		{
			$strSqlWhere = CSearch::__PrepareFilter([
				'MODULE_ID' => $MODULE_ID,
				$op . 'ITEM_ID' => $ITEM_ID,
				[
					'=PARAM1' => $PARAM1,
	...
	public static function ChangeSite($MODULE_ID, $arSite, $ITEM_ID = false, $PARAM1 = false, $PARAM2 = false, $SITE_ID = false)
	{
		...
		while ($arR = $r->Fetch())
		{
			CSearch::Update($arR['ID'], ['SITE_ID' => $arSite]);
		}
	}
	...
	public static function ChangePermission($MODULE_ID, $arGroups, $ITEM_ID = false, $PARAM1 = false, $PARAM2 = false, $SITE_ID = false, $PARAMS = false)
	{
		...
		while ($arR = $rs->fetch())
		{
			CSearch::Update($arR['ID'], ['PERMISSIONS' => $arGroups]);
		}
	}
}
...
class CSearchSQLHelper
{
	...
	function _CallbackURL($field_name, $operation, $field_value)
	{
		...
		{
			$sql_values = array_map([$DB, 'ForSQL'], array_filter($field_value));
		}
		elseif ($field_value !== false)
		{
			$sql_values = [$DB->ForSQL($field_value)];
		}
		else
		{
			$sql_values = [];
...
class CAllSearchQuery
{
	...
	function StemQuery($q, $lang = 'en')
	{
		...
		return preg_replace_callback('/([' . $arStemInfo['pcre_letters'] . ']+)/u', [$this, 'StemWord'], $q);
	}
