/* eslint-disable */
(function (exports,main_core,main_core_events,main_popup,ui_entitySelector) {
	'use strict';
	  ...
	  babelHelpers.createClass(VariationGrid, [{
	    ...
	    value: function onScrollHandler(event) {
	      ...
	      this.propertiesWithMenu.forEach(function (propertyId) {
	        var menu = main_popup.MenuManager.getMenuById("".concat(propertyId, "_menu"));
	        if (menu) {
	          menu.close();
	        }
	      });
	    ...
	    value: function onPrepareDropDownItems(event) {
	      ...
	      requestAnimationFrame(function () {
	        var popup = document.getElementById("menu-popup-".concat(menuId));
	    ...
	    value: function clearGridSettingsPopupStuff() {
	      main_core.Dom.remove(document.getElementById("".concat(this.gridId, "-grid-settings-window")));
	    }
	  }, {
	    ...
	    value: function getGrid() {
	      if (this.grid === null) {
	        if (!main_core.Reflection.getClass('BX.Main.gridManager.getInstanceById')) {
	          throw new Error("Cannot find grid with '".concat(this.gridId, "' id."));
	        }
	    ...
	    value: function openStoreAmountPopup(rowId, quantityNode) {
	      var popupId = "".concat(rowId, "-store-amount");
	    ...
	    value: function getStoreAmountTable(stores, rowId) {
	      ...
	      this.addCellToTable(tableHeadRow, main_core.Loc.getMessage('C_PVG_STORE_AMOUNT_POPUP_QUANTITY_COMMON1_MSGVER_1'), true);
	    ...
	    value: function destroyStoreAmountPopups() {
	      ...
	      rows.forEach(function (row) {
	        if (row.isBodyChild() && !row.isTemplate()) {
	          var _popup$destroy;
	          var popupId = "".concat(row.getId(), "-store-amount");
	          var popup = main_popup.PopupManager.getPopupById(popupId);
	    ...
	    value: function prepareNewRowData(newRowData) {
	      for (var i in newRowData) {
	        if (newRowData.hasOwnProperty(i) && (i.includes('[VIEW_HTML]') || i.includes('[EDIT_HTML]') || i.includes('XML_ID'))) {
	          delete newRowData[i];
	        }
	      }
	...
	function _getSimpleProductRestrictionContent() {
	  var text = main_core.Loc.getMessage('C_PVG_SIMPLE_PRODUCT_POPUP_TEXT_MSGVER_1', {
