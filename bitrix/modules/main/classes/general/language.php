...
class CAllLanguage
{
	...
	{
		return CLanguage::GetList('', '', ["LID" => $ID]);
	}
	...
	public function CheckFields($arFields, $ID = false)
	{
		...
		if (($ID === false || isset($arFields["LID"])) && mb_strlen($arFields["LID"]) <> 2)
		{
			...
			$arMsg[] = ["id" => "LID", "text" => GetMessage("BAD_LANG_LID")];
		}
		...
		if (isset($arFields["NAME"]) && mb_strlen($arFields["NAME"]) < 2)
		{
			...
			$arMsg[] = ["id" => "NAME", "text" => GetMessage("BAD_LANG_NAME")];
		}
		if (isset($arFields["SORT"]) && intval($arFields["SORT"]) <= 0)
		{
			$this->LAST_ERROR .= GetMessage("BAD_LANG_SORT") . " ";
			$arMsg[] = ["id" => "SORT", "text" => GetMessage("BAD_LANG_SORT")];
		}
		...
		if ($ID === false)
		{
			...
			if ($r->Fetch())
			{
				...
				$e = new CAdminException([["id" => "LID", "text" => GetMessage("BAD_LANG_DUP")]]);
				$APPLICATION->ThrowException($e);
				return false;
			}
		}
	...
	public static function Delete($ID)
	{
		...
		$db_res = CLang::GetList('', '', ["LANGUAGE_ID" => $ID]);
		if ($db_res->Fetch())
		{
			return false;
		}
		...
		CFavorites::OnLanguageDelete($ID);
