...
class PHPParser
{
	...
	public static function GetParamsRec($params, &$arAllStr, &$arResult)
	{
		...
		if ($found)
		{
			$arParams = static::GetParams($paramsList);
			foreach ($arParams as $i => $el)
			{
				...
				if ($p === false)
				{
					...
					{
						$arResult = static::ReplString($el, $arAllStr);
					}
					else
					{
						static::GetParamsRec($el, $arAllStr, $arResult[$i]);
					}
				}
				else
				{
					// value with index
					$el_ind = static::ReplString(mb_substr($el, 0, $p), $arAllStr);
					$el_val = mb_substr($el, $p + 2);
					static::GetParamsRec($el_val, $arAllStr, $arResult[$el_ind]);
				}
			}
		}
		else
		{
			$arResult = static::ReplString($params, $arAllStr);
		}
	}
		...
		if ($pos = mb_strpos($new_str, "("))
		{
			...
			switch (mb_strtoupper($func_name))
			{
				case '$APPLICATION->INCLUDEFILE(':
					...
					$arParams = static::GetParams($params);
					$arIncludeParams = [];

					if (preg_match("/^array\\(/i", $arParams[1]))
					{
						$arParams2 = static::GetParams(mb_substr($arParams[1], 6));
						foreach ($arParams2 as $el)
						{
							...
							{
								$res_ar = [];
								$arParamsN = static::GetParams(mb_substr($el_val, 6));
								foreach ($arParamsN as $param)
								{
									$res_ar[] = static::ReplString($param, $arAllStr);
								}
							...
							{
								$arIncludeParams[$el_ind] = static::ReplString($el_val, $arAllStr);
							}
						}
					}
					return [
						"SCRIPT_NAME" => static::ReplString($arParams[0], $arAllStr),
	...
	public static function GetComponentParams($instruction, $arAllStr)
	{
		if ($pos = mb_strpos($instruction, "("))
		{
			...
			if (!empty($arParams[2]))
			{
				static::GetParamsRec($arParams[2], $arAllStr, $arIncludeParams);
			}
			if (!empty($arParams[4]))
			{
				static::GetParamsRec($arParams[4], $arAllStr, $arFuncParams);
			}

			return [
				"COMPONENT_NAME" => static::ReplString($arParams[0] ?? '', $arAllStr),
				"TEMPLATE_NAME" => static::ReplString($arParams[1] ?? '', $arAllStr),
				"PARAMS" => $arIncludeParams,
				"PARENT_COMP" => $arParams[3] ?? '',
				"VARIABLE" => $var_name,
				"FUNCTION_PARAMS" => $arFuncParams,
				"RETURN_RESULT" => $arParams[5] ?? '',
			];
		}
		return [];
	}
				...
				if (!$bInString)
				{
					...
					if ($ch == ";")
					{
						if ($bInComponent)
						{
							...
							$arComponents[$componentNumber]["DATA"] = static::GetComponentParams(preg_replace("#[ \r\n\t]#", "", $instruction), $arAllStr);
						}
						$instruction = "";
						continue;
					}
		...
		while (($p = mb_strpos($filesrc, "<?", $p)) !== false)
		{
			...
			while ($i < $nLen - 1)
			{
				...
				if (!$bInString)
				{
					...
					if ($ch == "/" && $i + 1 < $nLen)
					{
						...
						if ($ti)
						{
							...
							if ($ti > $posnext && mb_substr($filesrc, $i + 1, 1) != "*")
							{
								...
								// вырежем скрипт
								$arScripts[] = [$p, $posnext, mb_substr($filesrc, $p, $posnext - $p)];
								break;
							}
							else
							{
					...
					{
						$i = $i + 2;
						$arScripts[] = [$p, $i, mb_substr($filesrc, $p, $i - $p)];
						break;
					}
				}
		...
		foreach ($arVals as $key => $val)
		{
			...
			if (is_array($val) && count($val) > 1)
			{
				...
				foreach ($val as $p)
				{
					...
					$zn .= "\t\t\t\t\t" . static::PreparePHP($p);
				}
				$res .= $zn . "\r\n\t\t\t\t),";
			}
			elseif (is_array($val))
			{
				$res .= "array(" . static::PreparePHP($val[0]) . ")," . $comm;
			}
			else
			{
				$res .= static::PreparePHP($val) . "," . $comm;
			}
		}
	...
	public static function ReturnPHPStrRec($arVal, $level, $comm = "")
	{
		...
		if (is_array($arVal))
		{
			...
			{
				$result .= $pref . "\t" . ((intval($key) . "|" == $key . "|") ? $key : static::PreparePHP($key)) . " => " . static::ReturnPHPStrRec($value, $level + 1);
			}
			$result .= $pref . "],\n";
		}
		else
		{
			$result .= static::PreparePHP($arVal) . "," . (($level == 1) ? $comm : "") . "\n";
		}
		return $result;
	}

	// Components 2. Return PHP string of component call params
	public static function ReturnPHPStr2($arVals, $arParams = [])
	{
		$res = "";
		foreach ($arVals as $key => $val)
		{
			...
			$res .= static::ReturnPHPStrRec($val, 1, $comm);
		}
	...
	public static function FindComponent($component_name, $filesrc, $src_line)
	{
		...
		$arComponents = static::ParseScript($filesrc);
	...
	public static function getPhpChunks($filesrc, $limit = false)
	{
		...
		{
			foreach (static::ParseFile($filesrc, $limit) as $chunk)
			{
				$chunks[] = $chunk[2];
			}
		}
	...
	public static function getPageTitle($filesrc, $prolog = false)
	{
		if ($prolog === false)
		{
			$chunks = static::getPhpChunks($filesrc, 1);
			if (!empty($chunks))
			{
				$prolog = &$chunks[0];
			}
	...
	public static function buildComponentCode(array $component, ?string $templateName = null, ?array $parameters = null): string
	{
		if ($templateName === null)
		{
			$templateName = $component['DATA']['TEMPLATE_NAME'];
		}
		if ($parameters === null)
		{
			$parameters = $component['DATA']['PARAMS'];
		}

		$functionParams = '';
		if (!empty($component['DATA']['FUNCTION_PARAMS']))
		{
			$functionParams = ",\n"
				. "\t[\n"
		...
		elseif (!empty($component['DATA']['RETURN_RESULT']))
		{
			$functionParams = ",\n\t[]";
		}

		$code = ($component['DATA']['VARIABLE'] ? $component['DATA']['VARIABLE'] . ' = ' : '')
			. "\$APPLICATION->IncludeComponent(\n"
			...
			. "\t],\n"
			. "\t" . (!empty($component['DATA']['PARENT_COMP']) ? $component['DATA']['PARENT_COMP'] : 'false')
			. $functionParams
			. (!empty($component['DATA']['RETURN_RESULT']) ? ",\n\t" . $component['DATA']['RETURN_RESULT'] : '')
			. "\n);";
