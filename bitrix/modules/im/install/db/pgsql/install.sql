...
CREATE INDEX ix_b_im_call_state_start ON b_im_call (state, start_date);
...
CREATE UNIQUE INDEX ix_b_im_anchor_1 ON b_im_anchor (MESSAGE_ID, type, user_id, from_user_id);
CREATE INDEX ix_b_im_anchor_2 ON b_im_anchor (user_id, chat_id);
...
CREATE INDEX ix_b_im_notify_group_user_id ON b_im_notify_group (user_id);
...
CREATE UNIQUE INDEX ux_b_im_notify_group_condition_group_id_module_event ON b_im_notify_group_condition (group_id, module, event);
