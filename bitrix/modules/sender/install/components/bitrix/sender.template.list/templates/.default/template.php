...
foreach ($arResult['ROWS'] as $index => $data)
{
	...
	$actions = array();
	$actions[] = array(
		'TITLE' => $arParams['CAN_EDIT'] ? Loc::getMessage('SENDER_TEMPLATE_LIST_BTN_EDIT_TITLE') : Loc::getMessage('SENDER_TEMPLATE_LIST_BTN_VIEW_TITLE'),
		'TEXT' => $arParams['CAN_EDIT'] ? Loc::getMessage('SENDER_TEMPLATE_LIST_BTN_EDIT') : Loc::getMessage('SENDER_TEMPLATE_LIST_BTN_VIEW'),
...
if ($arParams['CAN_EDIT'])
{
	...
	$button['ONCHANGE'][0]['DATA'][0]['JS'] = 'BX.Sender.TemplateList.removeSelected()';
